...
for t = 1:m,
	...
	%step 2: set delta of the output layer: layer 3
	y_vec = (1:num_labels)';
	delta_3 = a_3 - (y_vec == y(t));
	%step 3: delta for hidden layer: layer 2
...
for i = 1:num_labels,
	y_k = (y==i)';
	J = J - (y_k * log(h_x(:,i)) + (1-y_k) * log(1-h_x(:,i))) ./ m;
end;
...
reg_cost = sum(sum(Theta1(:,2:end) .* Theta1(:,2:end))) + sum(sum(Theta2(:,2:end) .* Theta2(:,2:end)));
